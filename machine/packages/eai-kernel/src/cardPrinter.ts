...
export function printCard(ssot: any, input: CardInput, generatorVersion = "2.1.0") {
  ...
  const evidence_links = input.evidence_pack ? suggestEvidenceLinks(input.context, gate.bands as BandSelection, input.evidence_pack) : [];
  ...
  const card: any = {
    ...
    policy,
    prompt_pack: {
      system_prompt: "Je bent een onderwijsassistent die strikt de EAI-card volgt. Je vraagt door, dwingt transparantie af, en je geeft geen verboden output.",
      user_prompt_template: "Gebruik de EAI-card hieronder. Start met 2 verduidelijkende vragen, daarna begeleid je de leerling stap voor stap.",
      paste_prompt_text: ""
    },
  ...
  card.prompt_pack.paste_prompt_text = buildPastePromptText(input.context, gate.bands as BandSelection, policy, outputContract);
