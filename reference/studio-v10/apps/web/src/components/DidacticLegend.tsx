...
const DidacticLegend: React.FC<DidacticLegendProps> = ({ isOpen, onClose }) => {
  ...
  const t = {
      ...
      modes: [
          { color: 'bg-blue-600', text: 'text-blue-400', border: 'border-blue-500/30', bgLight: 'bg-blue-500/10', label: 'Focus & Structuur', desc: 'De standaard modus. De AI helpt je stof te ordenen, geeft uitleg en bewaakt de grote lijn. Hier ben je aan het bouwen.' },
          { color: 'bg-red-600', text: 'text-red-500', border: 'border-red-500/30', bgLight: 'bg-red-500/10', label: 'Advocaat van de Duivel', desc: 'Kritische modus. De AI valt je argumenten aan, zoekt zwakke plekken en dwingt je om scherper na te denken. Geen genade.' },
            ...
            <div className="p-4 border-b border-white/5 bg-[#0b1120]">
                ...
                <div className="flex gap-2 p-1 bg-black/20 rounded-lg">
                    <button 
                        ...
                        className={`flex-1 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded transition-all ${activeTab === 'KENNIS' ? 'bg-slate-700 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                        Kennis (Theorie)
                    </button>
                    <button 
                        onClick={() => setActiveTab('MODI')}
                        className={`flex-1 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded transition-all ${activeTab === 'MODI' ? 'bg-slate-700 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                        Modi (Praktijk)
                    </button>
                </div>
