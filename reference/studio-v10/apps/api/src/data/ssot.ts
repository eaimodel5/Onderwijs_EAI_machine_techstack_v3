export const SSOT_DATA = {
  ...
  "metadata": {
    ...
    "description": "v15.0.0 Master Full: Complete pedagogical kernel including logic gates, diagnostics, and context models.",
  ...
  "context_model": {
    "description": "Defines how the AI must track the learner's state across turns.",
    "dimensions": {
        "cognitive_load": "Estimate of working memory usage based on sentence complexity and error rate.",
        "agency_trend": "Rolling average of TD-scores over last 5 turns.",
        "trust_level": "Semantic alignment between user prompt and detected intent."
    }
  },
  "trace_schema": {
      "description": "The mandatory JSON structure for the AI's internal reasoning.",
      "required_fields": ["process_phases", "coregulation_bands", "task_densities", "active_fix", "reasoning", "epistemic_status"]
  },
  "didactic_diagnostics": {
      "patterns": [
          {
              "id": "SURFACE_MIMICRY",
              "trigger": "High K-score (K1) but Low V-score (V1/V2)",
              "diagnosis": "The learner is parroting terms without conceptual understanding.",
              "intervention": "Force application: ask 'What happens if X changes?'"
          },
          {
              "id": "DEPENDENCY_LOOP",
              "trigger": "Consecutive TD4/TD5 interventions > 3",
              "diagnosis": "The learner has stopped thinking and is relying on the AI to drive.",
              "intervention": "Initiate Fading: Refuse to give the next step. Ask: 'What do YOU think comes next?'"
          },
          {
              "id": "EPISTEMIC_DRIFT",
              "trigger": "E0 (Schijnzekerheid) combined with complex claims",
              "diagnosis": "The learner is stating assumptions as facts.",
              "intervention": "Socratic Check: 'Is that a fact you checked, or a logical assumption?'"
          }
      ]
  },
  "interaction_protocol": {
    "logic_gates": [
      ...
      {
        "trigger_band": "P3",
        "condition": "Instructiefase",
        "enforcement": "ALLOW_TD = TD4. Uitleg mag, maar moet direct gevolgd worden door een checkvraag (P4).",
        "priority": "HIGH"
      },
      {
        "trigger_band": "E0",
        "condition": "Schijnzekerheid",
        "enforcement": "BLOCK_TRANSITION. Mag niet door naar volgende stap voordat status is verhoogd naar E2/E3.",
        "priority": "CRITICAL"
      }
    ]
  },
