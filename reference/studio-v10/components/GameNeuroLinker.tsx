...
const GameNeuroLinker: React.FC<GameNeuroLinkerProps> = ({ onClose }) => {
  ...
  const spawnNode = () => {
    ...
    state.current.nodes.push({ id: Date.now() + Math.random(), angle: angle, life: 0, type: type });
  };
  ...
  const createExplosion = (x: number, y: number, color: string) => {
      for (let i = 0; i < 8; i++) {
          ...
          state.current.particles.push({ id: Math.random(), x, y, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed, life: 1.0, color });
      }
  };
  ...
  const restartGame = (e?: React.SyntheticEvent) => {
      if(e) { e.preventDefault(); e.stopPropagation(); }
      state.current = { rotation: 0, speed: 2, level: 1, nodes: [], particles: [], lastSpawn: 0, isPlaying: true, score: 0, stability: 100 };
      setScore(0);
      setLevel(1);
      setStability(100);
      setGameOver(false);
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
        ...
        ctx.strokeStyle = '#1e293b'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(centerX, centerY, radius, 0, Math.PI * 2); ctx.stroke();
        ctx.strokeStyle = '#0f172a'; ctx.lineWidth = 1; ctx.beginPath(); ctx.arc(centerX, centerY, radius * 0.7, 0, Math.PI * 2); ctx.stroke();
        ...
        state.current.nodes = state.current.nodes.filter(node => {
            ...
            ctx.fillStyle = color; ctx.shadowColor = color; ctx.shadowBlur = 10; ctx.beginPath(); ctx.arc(x, y, size, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0;
            if (node.life > 100) { ctx.strokeStyle = '#ef4444'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(x, y, size + 4, 0, Math.PI * 2); ctx.stroke(); }
            return true;
        });
        ...
        grad.addColorStop(0, 'rgba(56, 189, 248, 0)'); grad.addColorStop(1, 'rgba(56, 189, 248, 1)');
        ctx.strokeStyle = grad; ctx.lineWidth = 4; ctx.lineCap = 'round'; ctx.beginPath(); ctx.moveTo(centerX, centerY); ctx.lineTo(tipX, tipY); ctx.stroke();
        ctx.fillStyle = '#ffffff'; ctx.shadowColor = '#06b6d4'; ctx.shadowBlur = 15; ctx.beginPath(); ctx.arc(tipX, tipY, 4, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0;

        state.current.particles = state.current.particles.filter(p => {
            p.x += p.vx; p.y += p.vy; p.life -= 0.05;
            ctx.fillStyle = p.color; ctx.globalAlpha = Math.max(0, p.life); ctx.beginPath(); ctx.arc(p.x, p.y, 2, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1.0;
            return p.life > 0;
        });

        if (state.current.stability <= 0 && state.current.isPlaying) { endGame(); }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
      <div className="bg-[#0b1120] border border-cyan-500/30 rounded-2xl shadow-[0_0_50px_rgba(6,182,212,0.15)] p-4 sm:p-6 w-full max-w-md relative overflow-hidden flex flex-col h-[90vh] sm:h-auto">
        <div className="flex justify-between items-center mb-4 z-10 shrink-0">
            ...
            <button onClick={onClose} className="p-3 bg-white/5 hover:bg-white/10 rounded-full text-slate-300 hover:text-white transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
        </div>
        <div className="relative w-full aspect-square bg-[#0f172a]/50 rounded-full border border-slate-800 overflow-hidden mb-4 shrink-0 touch-none select-none" onPointerDown={handleAction}>
            ...
            {gameOver && (
                <div className="absolute inset-0 z-20 bg-black/80 flex flex-col items-center justify-center text-center p-4 backdrop-blur-sm animate-in fade-in">
                    ...
                    <button onPointerDown={restartGame} className="px-8 py-3 bg-cyan-600 hover:bg-cyan-500 text-white font-bold text-sm rounded-full shadow-[0_0_20px_rgba(6,182,212,0.4)] transition-all active:scale-95 tracking-widest pointer-events-auto">REBOOT</button>
                </div>
            )}
        </div>
        <div className="mb-2 shrink-0">
            ...
            <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                <div className={`h-full transition-all duration-300 ${stability < 30 ? 'bg-red-500 shadow-[0_0_10px_#ef4444]' : 'bg-cyan-500 shadow-[0_0_10px_#06b6d4]'}`} style={{ width: `${Math.max(0, stability)}%` }}></div>
            </div>
        </div>
      </div>
    </div>
