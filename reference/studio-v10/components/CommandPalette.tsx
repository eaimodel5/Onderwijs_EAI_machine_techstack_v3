      ...
      <div className="bg-[#0f172a] w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh] border border-slate-700 animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-2 scrollbar-hide bg-[#0f172a]">
          {filteredCommands.map((cmd) => (
            <button
              key={cmd.command}
              onClick={() => { onSelectCommand(cmd.command); onClose(); }}
            ...
            >
              <div className="flex items-center justify-between w-full mb-1">
                <code className="text-cyan-400 font-mono font-bold text-xs bg-cyan-900/20 px-2 py-0.5 rounded border border-cyan-500/20 group-hover:bg-cyan-900/40">{cmd.command}</code>
              </div>
              <p className="text-xs text-slate-400 line-clamp-2 pl-1">{cmd.description}</p>
            </button>
          ))}
          {filteredCommands.length === 0 && <div className="p-8 text-center text-slate-600 italic text-xs">Geen resultaten gevonden in SSOT.</div>}
        </div>
