...
export interface MechanicalState {
  ...
  repairLog?: { timestamp: number; error: string; brokenPayload: string };
  supervisorLog?: { timestamp: number; breach: LogicGateBreach; original_reasoning: string; correction_prompt: string };
  softValidationLog?: string[];
  logicGateBreach?: LogicGateBreach;
  routerDecision?: { target_model: string; thinking_budget: number; intent_category: 'FAST' | 'MID' | 'SLOW'; reasoning: string };
