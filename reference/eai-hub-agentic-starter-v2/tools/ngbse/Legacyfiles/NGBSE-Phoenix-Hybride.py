# setup_ngbse_final.py
import os
import shutil
import textwrap
from pathlib import Path

print(">>> Bouwen van NGBSE-FINAL: De Volledige, Onverkorte Hybride Engine <<<")
root = Path("./NGBSE-FINAL")
...
(root / "data/offline_example.jsonl").write_text('{"source":"manual_input","query":"example query","url":"http://example.com","timestamp":"2023-01-01T12:00:00Z","hash":"manual123"}')
...
def main():
    parser = argparse.ArgumentParser(description="NGBSE-FINAL Engine")
    ...
    if args.offline:
        ...
        ingest_path = Path(args.offline)
        if not ingest_path.is_dir(): print(f"FATAL: Map '{ingest_path}' bestaat niet."); return
        raw_findings = ingest_from_directory(ingest_path, cfg)
    else:
    ...
    print(f"\\n>>> NGBSE-FINAL Run Voltooid. Rapport: {report_path} <<<")
...
def collect(seed_obj, cfg):
    ...
    if not api_key: print("INFO: Urlscan collector overgeslagen (API-sleutel vereist in deze modus)."); return
    try:
...
def build_report(findings, synthesis):
    ...
    for f in findings[:20]:
        r=table.add_row().cells; r[0].text=f'{f.get("score",0):.4f}'; e=f.get('enrichment',{}); r[1].text=f'M={e.get("M")} C={e.get("C")} Q={e.get("Q")}'
...
# Checkt of het eerste argument --offline is en geeft het door
if [[ "$1" == "--offline" ]]; then
