# setup_ngbse13_singularity.py
import os
import shutil
import textwrap
from pathlib import Path

print(">>> Bouwen van NGBSE 13.0 'Singularity Engine': De Finale, Volledige Synthese <<<")
root = Path("./NGBSE_13_SINGULARITY")
...
# NGBSE 13.0 "Singularity" Configuratie
orchestrator:
...
def main():
    ...
    log = get_logger("NGBSE-13.0", "out/run.log.jsonl")
    load_dotenv()
    ...
    if cfg.get("exports", {}).get("stix21"): export_to_stix(all_scored_findings, synthesis, "NGBSE-13.0", str(out_dir / "findings.stix.json"))
        
    report_path = build_report(scored_assets, synthesis, blindspots)
    log.info(f"NGBSE 13.0 Run Voltooid. Rapport: {report_path}")
    print(f"\\n>>> NGBSE 13.0 Run Voltooid. Rapport: {report_path} <<<")
...
def validate_assets(asset_clusters, allowlist, log):
    ...
    for asset_key, asset_data in asset_clusters.items():
        ...
        try:
            r = requests.head('https://' + asset_key, timeout=7, allow_redirects=True, headers={'User-Agent':'NGBSE-13.0'})
...
def get_dynamic_params(asset_data, base_defaults):
    ...
    # De basis-validiteit wordt beïnvloed door live validatie.
    base_v = 1.0 if asset_data.get('validated') else 0.5
    # Als er veel corroboratie is (hoge C), is ons vertrouwen in de validiteit hoger.
    if enrich.get('C', 0) >= 3:
...
def query_llm(prompt, config, log):
    ...
    try:
        log.info("LLM aangeroepen voor synthese...", {"model": model})
        r = requests.post(endpoint, json={"model": model, "prompt": prompt, "stream": False}, timeout=180)
        r.raise_for_status()
        return r.json().get("response", "Error: Geen 'response' veld in LLM output.")
    except requests.RequestException as e:
        log.error(f"Kon geen verbinding maken met lokale LLM op {endpoint}", {"error": str(e)})
        return f"Error: Kon geen verbinding maken met de lokale LLM. Zorg ervoor dat Ollama draait."
...
def normalize_asset(finding):
    ...
    try:
        ...
        if any(ext in p.path for ext in ['.yml','.json','.config']):
...
def write_csv(findings, path):
    ...
    with open(path, 'w', newline='', encoding='utf-8') as h:
        w = csv.DictWriter(h, fieldnames=all_keys, extrasaction='ignore')
        w.writeheader()
...
def export_to_stix(findings, synthesis, author_name, out_path):
    ...
    for f in findings:
        ...
        indicator = Indicator(
            name=f"OSINT Finding: {url}", pattern_type="stix", pattern=pattern,
    ...
    stix_report = Report(
        name=f"NGBSE 13.0 Synthesis Report ({datetime.datetime.now().strftime('%Y-%m-%d')})",
        description=synthesis_desc, object_refs=report_refs,
        created_by_ref=identity.id, report_types=["threat-report"]
    )
...
def build_report(scored_assets, synthesis, blindspots):
    out_path = Path("out/NGBSE_13_Report.docx")
    doc = Document()
    doc.add_heading("NGBSE 13.0: Singularity Engine Briefing", 0)
    ...
    doc.add_heading('Top Risico-Assets', 1)
    table = doc.add_table(1, 4)
    table.style = 'Table Grid'
    hdr = table.rows[0].cells
    hdr[0].text = 'E_AI*'; hdr[1].text = 'Asset'; hdr[2].text = 'Info (M,C,Q)'; hdr[3].text = 'Bewijs'
    sorted_assets = sorted(scored_assets.values(), key=lambda x: x.get('score', 0), reverse=True)
    for asset in sorted_assets[:20]:
        r = table.add_row().cells
        r[0].text = f'{asset.get("score",0):.4f}'
        r[1].text = list(asset['urls'])[0] if asset.get('urls') else 'N/A'
        e = asset.get('enrichment', {})
        r[2].text = f"M={e.get('M')} C={e.get('C')} Q={e.get('Q'):.2f}"
...
#!/bin/bash
set -e
echo ">>> NGBSE 13.0: Singularity Engine starten..."
if [ ! -f .env ]; then
    cp .env.example .env
...
print("\\n>>> SETUP VOLTOOID. NGBSE 13.0 'Singularity Engine' is gebouwd. <<<")
